    ...
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900  md:mx-10' >
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 gap-y-6 pt-5 px-3 sm:px-0">
            {doctors.slice(0,10).map((item,index)=>(
             <div onClick={()=>navigate(`/patient/appointment/${item._id}`)} key={index} className='border border-green-500 rounded-xl max-w-441 overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500 ' >
        ...
        <button onClick={()=>{navigate('/patient/searchdoctor/'); scrollTo(0,0)}} className='bg-green-500 text-gray-600 px-12 py-3 rounded-full mt-10  cursor-pointer'>More</button>
    </div>
  )
}
